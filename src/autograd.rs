...
use super::tensor::{Tensor, SharpTensor};
...
impl <T: SharpTensor> Autograd<T> {
    pub fn new(matrix: T) -> Self {
        Self(Rc::new(
            Computation { tensor: matrix, grad: RefCell::new(None), op: None }
        ))
    }
...
impl <T: SharpTensor> Tensor for Autograd<T> {
    ...
    fn add(self, other: &Self) -> Option<Self>{
        Operation::Add { lhs: self.0, rhs: other.0.clone() }.forward()
    }

    fn sub(self, other: &Self) -> Option<Self> {
        Operation::Sub { lhs: self.0, rhs: other.0.clone() }.forward()
    }

    fn mul(self, other: &Self) -> Option<Self> {
        Operation::Mul { lhs: self.0, rhs: other.0.clone() }.forward()
    }

    fn dot(self, other: &Self, depth: usize) -> Option<Self> {
        Operation::Dot { lhs: self.0, rhs: other.0.clone(), depth }.forward()
    }
...
enum Operation<T: SharpTensor> {
    Add { lhs: AutogradNode<T>, rhs: AutogradNode<T> },
    Sub { lhs: AutogradNode<T>, rhs: AutogradNode<T> },
    Mul { lhs: AutogradNode<T>, rhs: AutogradNode<T> },
    Dot { lhs: AutogradNode<T>, rhs: AutogradNode<T>, depth: usize },
    Sum { t: AutogradNode<T> },
    Ln { t: AutogradNode<T> },
    Exp { t: AutogradNode<T> },
    Pow { t: AutogradNode<T>, i: i32 },
    Neg { t: AutogradNode<T> },
    Max { t: AutogradNode<T>, u: f64 },
}

impl <T: SharpTensor> Operation<T>{
    fn arithmetic_backward(lhs: &AutogradNode<T>, rhs: &AutogradNode<T>, grad: &T, op: impl Fn(&f64, &f64) -> (f64, f64)) {
        let mut lhs_grad = T::tensor(Fill { shape: lhs.tensor.shape().to_vec(), with: 0.0 }).unwrap();
        let mut rhs_grad = T::tensor(Fill { shape: rhs.tensor.shape().to_vec(), with: 0.0 }).unwrap();

        let mut new_point = vec![0; grad.ndim()];
        'iterate: loop {
            let lhs_point: Vec<usize> = new_point.iter().enumerate().filter(|(i, _)| *i < lhs.tensor.ndim()).map(|(i, x)| x % lhs.tensor.shape()[i]).collect();
            let rhs_point: Vec<usize> = new_point.iter().enumerate().filter(|(i, _)| *i < rhs.tensor.ndim()).map(|(i, x)| x % rhs.tensor.shape()[i]).collect();
            let (lhs_lgrad, rhs_lgrad) = op(lhs.tensor.get(&lhs_point).unwrap(), rhs.tensor.get(&rhs_point).unwrap());
    ...
    fn forward(self) -> Option<Autograd<T>> {
        ...
        tensor
            .map(|tensor| Autograd(Rc::new(Computation {
                tensor,
    ...
    fn backward(&self, grad: &T) {
        match self {
            ...
            Self::Dot { lhs, rhs, depth } => {
                ...
                lhs.backward(
                    grad.clone().dot(&rhs.tensor.clone().tranpose(&rhs_axes).unwrap(), lhs_shift).unwrap()
                );
                ...
                rhs.backward(
                    lhs.tensor.clone().tranpose(&lhs_axes).unwrap().dot(grad, rhs_shift).unwrap()
                );
            }
            Self::Sum { t } => {
                t.backward(T::tensor(Fill { shape: t.tensor.shape().to_vec(), with: *grad.get(&[]).unwrap() }).unwrap())
            }
            Self::Ln { t } => {
            ...
            Self::Pow { t, i } => {
                t.backward(t.tensor.clone().pow(i - 1).mul(&Tensor::scalar(*i as f64)).unwrap().mul(grad).unwrap());
            }
            Self::Neg { t } => {
            ...
            Self::Max { t, u } => {
                ...
                loc.iter_mut().for_each(|x| *x = if *x >= *u { 1.0 } else { 0.0 } );
